...
ggplot(data = m.safe) + geom_line(mapping = aes(x = time, y = value, color = variable)) + 
  scale_x_continuous(name ="Year", breaks = seq(from=0,to=20,by=5), label = as.character(2019 + seq(from=0,to=20,by=5)), expand = c(0,0)) +theme_bw(base_size = 16)+theme(plot.title = element_text(hjust = 0.5))+
  ...
  labs(color = "Autonomy") + ggtitle("Safelife Marketshare Base Scenario") 
...
ggplot(data = tmp.melt) + geom_bar(mapping = aes(x = time, y = value, fill = variable) , stat = "identity", width = 0.2) +
  ggtitle("Pure_Premium") +theme(plot.title = element_text(hjust = 0.5))+ 
  scale_y_continuous(name = "Total amount in billions" , breaks = seq(from=0,to=3.000,by=0.200)*10^9, labels = seq(from=0,to=3.000,by=0.200), limits=c(0,3*10^9), expand=c(0,0) ) +
